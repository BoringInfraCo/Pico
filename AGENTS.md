...
- **v0.5 Continuous and Runtime Observation EXTEND** (ROADMAP §24 phase-exit review). Delivered slices: S037 `pico watch` (opt-in foreground FILESYSTEM_CHANGE observer over adapter-read configs), S038 deterministic change notices + read-only `pico status`, S039 `pico runtime` observability survey (metadata-only; `approved vs denied use` = `NOT_AVAILABLE`), S040 opt-in `pico scan --runtime` content-free runtime evidence ingestion (workspace-scoped; promotes the golden-path capability configured `DERIVED` → observed `CONFIRMED`, freshness-gated), S041 observed-execution explanation (scan summary `Observed execution:` / `Attempted (not executed):`, finding-detail `Observation basis`, MCP mirror), S042 observation basis on finding summaries (`pico findings` + MCP `list_findings`), S043 Claude Code observability probe (surface/`*.jsonl` count/CLI version; parsing DEFERRED). **The phase is not adjudicated complete.** Outstanding before any advance: detection latency and overhead are unmeasured; observer downtime is not visible (silence is not surfaced as "not observing"); `.pico/watch.jsonl` has no retention/prune or documented deletion path (unbounded); no real (non-synthetic) dogfood shows a changed decision; the carried v0.3/v0.4 independent comprehension gates remain NOT RUN and no v0.5 protocol exists. Non-goals remained CLEAN in review: no daemon, no content capture, no mutation, no hook installation, no telemetry, no LLM in the truth path, no new dependencies. A third agent remains architecture unless concrete user demand outweighs this sequencing.
