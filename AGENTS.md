...
## Current baseline: v0.1–v0.4 advanced with caveats; v0.5 reviewed EXTEND; v0.6 not authorized
...
- **v0.5 Continuous and Runtime Observation EXTEND** (ROADMAP §24 decision; §25 closeout progress). Delivered: S037 `pico watch` (opt-in foreground FILESYSTEM_CHANGE observer over adapter-read configs), S038 deterministic change notices + read-only `pico status`, S039 `pico runtime` observability survey (metadata-only; `approved vs denied use` = `NOT_AVAILABLE`), S040 opt-in `pico scan --runtime` content-free runtime evidence ingestion (workspace-scoped; configured `DERIVED` → observed `CONFIRMED`, freshness-gated), S041 observed-execution explanation, S042 observation basis on finding summaries, S043 Claude Code observability probe (parsing deferred), S044 closeout (measurements recorded in `dogfood/evidence-v0.5-measurements.md`; `pico status` reports `watching` / `NOT OBSERVING` / `no watch record`; `.pico/watch.jsonl` bounded by the observer and by `pico prune`; §9 distinctions wording amended). **Still open (human-required / refined by S045):** S045 ran the item-5 dogfood with a real OpenCode invocation — a real observation occurred (`Observed execution: can_execute via runtime evidence (FRESH)`, `DERIVED → CONFIRMED`) but changed no documented decision or remediation priority (`Findings: 0`), so item 5 is **NOT MET** (`dogfood/evidence-v0.5.md`). S045 also found a **HIGH defect**: `immutable=1` ignores the WAL, so a live store scanned without a checkpoint reports `Runtime evidence: UNSUPPORTED` and misses the freshest evidence. Item 6 remains **NOT RUN** (no independent participant; an implementing agent is ineligible) — the protocol and runbook are drafted, not administered. Accuracy, CPU/IO/memory, and sustained-operation figures remain unmeasured. No v0.6 scope is authorized. A third agent remains architecture unless concrete user demand outweighs this sequencing.
