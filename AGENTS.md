...
- **v0.5 Continuous and Runtime Observation IN PROGRESS** (S037 `pico watch`: opt-in foreground FILESYSTEM_CHANGE observer over adapter-read configs; S038 deterministic change notices + read-only `pico status`; S039 `pico runtime` observability survey — metadata-only, `approved vs denied use` reported `NOT_AVAILABLE`; S040 opt-in `pico scan --runtime` content-free runtime evidence ingestion — workspace-scoped, promotes the golden-path capability from configured `DERIVED` to observed `CONFIRMED`, freshness-gated; S041 observed-execution explanation: `pico scan` prints an `Observed execution:` / `Attempted (not executed):` line, `pico finding <id>` gains an `Observation basis` section, and MCP `get_finding` mirrors it — all derived from persisted evidence with no fingerprint or contract change. Finding-level evidence and MCP parity verified; no Cargo change. S042 observation basis on finding summaries (`pico findings` + MCP `list_findings`). S043 Claude Code observability probe: reports transcript surface/`*.jsonl` count/CLI version and DEFERS parsing (format documented as internal and version-varying; no local transcripts; no reliable version marker) — no Claude evidence or `observed` claim. No daemon, notification delivery, or enforcement. Known limitation: findings-level attempted basis is unreachable by design (S040 keeps attempted evidence unlinked); the v0.5 phase-exit review (ROADMAP §17) has not been run. A third agent remains architecture unless concrete user demand outweighs this sequencing.
